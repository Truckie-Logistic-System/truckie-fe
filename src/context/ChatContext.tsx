...
import type { ChatMessageDTO, MessageRequest, FirestoreTimestamp } from "@/models/Chat";
import type { CreateRoomResponse } from "@/models/Room";
import { mapChatMessageDTOArrayToUI, mapChatMessageDTOToUI, type ChatMessage } from '@/utils/chatMapper';
...
const loadMessagesForRoom = useCallback(async (roomId: string) => {
    ...
    try {
        ...
        const supportRoomsData: SupportRoom[] = rooms.map(room => ({
            ...
            type: (room.type === 'SUPPORT' || room.type === 'SUPPORTED') 
...
const joinRoom = async (roomId: string) => {
  ...
  const timestampToMillis = (timestamp: any): number => {
    ...
    if (typeof timestamp === 'object' && typeof timestamp.seconds === 'number') {
      return (timestamp.seconds * 1000) + Math.floor((timestamp.nanos || 0) / 1000000);
    }
    return 0;
  };
  ...
  const buildConversation = useCallback((
    ...
    return {
      ...
      lastMessageTime: lastMessage ? timestampToMillis(lastMessage.createAt).toString() : "",
      unreadCount: 0,
    };
  }, [timestampToMillis]);
  ...
  const handleNewMessage = useCallback((msg: ChatMessageDTO, roomId: string) => {
      ...
      prev.map((conv) =>
        conv.roomId === roomId
          ? {
              ...
              unreadCount: activeConversation?.roomId === roomId && isOpen
                ? conv.unreadCount
                : conv.unreadCount + 1,
            }
          : conv
  ...
  const openChat = useCallback(async (userId: string, roomId: string) => {
    try {
      const rooms = await roomService.getAllRoomsByUserId(userId);
  ...
  const loadMoreMessages = useCallback(async (roomId: string) => {
    ...
    try {
      const lastMessageId = activeConversation.messages.length > 0 
      ...
      if (chatPage.messages.length > 0) {
        ...
        setUiMessages(prev => {
          ...
          return unique.sort((a, b) => parseInt(a.timestamp) - parseInt(b.timestamp));
        });
      }
    } catch (error) {
  ...
  const connectWebSocket = useCallback((userId: string, roomId: string) => {
    ...
    stompClient.onConnect = (frame) => {
      ...
      stompClient.subscribe(`/topic/room/${roomId}`, (message: IMessage) => {
        try {
          ...
          if (!msg.createAt) {
            msg.createAt = { seconds: Math.floor(Date.now() / 1000), nanos: (Date.now() % 1000) * 1000000 };
          }
  ...
  const setChatMessages = useCallback((messages: ChatMessageDTO[]) => {
