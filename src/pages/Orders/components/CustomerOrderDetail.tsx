...
import { App, Button, Typography, Skeleton, Empty, Tabs, Card, Descriptions, Tag, Timeline, Image, Row, Col } from "antd";
import {
    ArrowLeftOutlined,
    InfoCircleOutlined,
    FileTextOutlined,
...
const CustomerOrderDetail: React.FC = () => {
    ...
    const [orderData, setOrderData] = useState<CustomerOrderDetailResponse["data"] | null>(null);
            ...
            >
                {order.orderDetails.map((detail, index) => (
                    <TabPane
                        tab={
                            <span>
                                <BoxPlotOutlined /> Chi tiết {index + 1} {detail.trackingCode ? `(${detail.trackingCode})` : ''}
                            </span>
                        }
                        key={index.toString()}
                    >
