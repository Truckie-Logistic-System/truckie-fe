...
const StaffChatConversationItem: React.FC<StaffChatConversationItemProps> = ({
    ...
    const { customerName, lastMessage, lastMessageTime, unreadCount, status } = conversation;
    const formattedTime = dayjs(lastMessageTime).format('HH:mm');
    ...
    return (
        <div
            className={`flex items-center p-3 border-b cursor-pointer hover:bg-gray-50 transition-colors ${isActive ? 'bg-blue-50' : ''
                }`}
            onClick={onClick}
        >
