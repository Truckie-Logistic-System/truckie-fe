...
interface CustomerContractExportContentProps {
...
const CustomerContractExportContent: React.FC<CustomerContractExportContentProps> = ({
  contractData,
  contract,
  contractSettings,
}) => {
  const effectiveDate = contract?.effectiveDate || new Date().toISOString().split('T')[0];
  const expirationDate = contract?.expirationDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const adjustedValue = contract?.adjustedValue ? parseContractValue(contract.adjustedValue) : 0;
  ...
  return (
    <div className="contract-export-content" style={{ padding: '20px', fontFamily: 'Times New Roman, serif' }}>
      ...
      <div className="header">
        ...
        ></div>
        <div className="title" style={{ marginTop: "30pt" }}>
          HỢP ĐỒNG DỊCH VỤ LOGISTICS
        </div>
        <div className="subtitle">Số: {contractData.orderInfo.orderCode}</div>
        <div className="subtitle">Mã hợp đồng: {contract?.id || contractData.contractId}</div>
      </div>
      ...
      <div className="terms-section">
        ...
        <p>1.2. Dịch vụ bao gồm: Vận chuyển hàng hóa từ điểm lấy hàng đến điểm giao hàng theo yêu cầu của Bên B.</p>
      </div>
      ...
      <div className="terms-section">
        ...
        <p>- Cung cấp bảo hiểm hàng hóa theo tỷ lệ quy định</p>
      ...
      <div className="terms-section">
        ...
        <p>6.4. Hợp đồng có hiệu lực kể từ ngày ký và thanh toán đặt cọc.</p>
        <div>
      ...
      <div className="signature-section">
        <div className="signature-box">
          ...
          <p style={{ marginTop: "60pt" }}>________________</p>
...
export default CustomerContractExportContent;
